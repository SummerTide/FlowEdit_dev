...
Uses standard StableDiffusion3ControlNetPipeline (which VAE-encodes the control image).
Uses enable_model_cpu_offload to fit in limited GPU memory.

Usage:
    python validate_controlnet.py \
        --controlnet_path ./checkpoints/sd3_controlnet_rs/checkpoint-200 \
...
def main():
    ...
    controlnet = SD3ControlNetModel.from_pretrained(args.controlnet_path, torch_dtype=torch.bfloat16)
